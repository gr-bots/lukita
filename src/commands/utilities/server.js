...
export default class ServerCommand extends Command {
  ...
  async execute({ interaction }) {
    if (await this.client.db.user.findOne({_id: this.interaction.user.id}).then(x => x.bl) == true) {
      await interaction.reply({ content: `> ⚠️・<@${this.interaction.user.id}>, Err... Parece que têm alguém na Blacklist..\n> Você está bloqueado de usar meus comandos.`, ephemeral: true, fetchReply: true });
    } else {

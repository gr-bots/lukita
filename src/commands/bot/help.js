...
export default {
    ...
    run: async (client, interaction) => {
        ...
        let embedHelp = new MessageEmbed()
            ...
            .setDescription(`**${interaction.member.user.tag}** seja bem vindo a minha central de ajuda e help dos meus comandos.\n> Abaixo está todos eles com contagem total separado por categorias, espero que você goste. :wink: \n\n${client.emotes.categBot} › Informações (Bot) \`[ ${client.commands.filter(a => a.category === "Bot").size} ]\` \n${codeBlock(client.commands.filter(a => a.category === "Bot").map(a => a.name).toString().replaceAll(",", " - " ) )}`)
